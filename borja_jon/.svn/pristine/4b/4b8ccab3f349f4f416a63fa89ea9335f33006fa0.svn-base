...
public class VideoController extends HttpServlet {
	...
	public static final String OP_GUSTA = "5";
	...
	private void eliminar(HttpServletRequest request, HttpServletResponse response) {
		...
		if(!videoDAO.videoPropietario(id, idUsuario)) {
			request.setAttribute("mensaje", new Alert("danger", "ERROR. EL video no te pertenece"));
		}else if (videoDAO.delete(id)) {
	...
	private void guardar(HttpServletRequest request, HttpServletResponse response) {
		...
		if(request.getParameter("usuario_id")==null) {
			...
			idUsuario = Integer.parseInt(request.getParameter("usuario_id"));
		}
		...
		if (violations.isEmpty()) {
			...
			try {
				...
				if (v.getId() == -1) {
					...
					if(videoDAO.videoPropietario(idVideo, idUsuario)) {
